...
#
# Examples:
# API KEYYYYYYYYYYY: AIzaSyDptB7mIn69d3vF4ek94q-w3-z0Y4cSeCk
...
99.times do
    ...
    long1 = data_hash['addresses'].fetch(i)['lng']
    lat1 = data_hash['addresses'].fetch(i)['lat']
    address = Address.new(
    ...
    customer = Customer.new(
        ...
        map = Map.new(
            location_of_building: address.number_and_street,
            number_of_floors: column.number_of_floors_served,
            client_name: customer.company_name,
            number_of_batteries: 1,
            number_of_columns: 1,
            number_of_elevators: 1,
            full_name_of_technical_contact: building.full_name_of_the_technical_contact_for_the_building,
            long: address.long,
            lat: address.lat
        )
        map.save
...
#Seeding for Address table from addresses.json file containing 100 addresses
#Create Address object to define variables coming from json file AdminUser.create!(email: 'admin@example.com', password: 'password', password_confirmation: 'password') if Rails.env.development?